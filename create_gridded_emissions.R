...
# --------------------------------------------------
create.gridded.emissions <- function(scenario.input.list) {
  library(rgdal)
  library(raster)
  library(plyr)
  
  # unpack the input list
  scenario_name <- scenario.input.list$scenario.name
  scenario.output.folder <- scenario.input.list$scenario.output.folder
  ...
  # If necessary create the output folder write the raster to an ascii file
  if (!(dir.exists(scenario.output.folder))) {
    dir.create(scenario.output.folder, recursive = T)
  }
  emis.raster.file <- file.path(scenario.output.folder, paste0("emis_", pollutant, ".asc"))
  writeRaster(nox.emis.raster, filename = emis.raster.file, overwrite=TRUE)
  png(file.path(scenario.output.folder, paste0(scenario_name, "_", pollutant, "_emissions.png")))
  plot(log(nox.emis.raster), main = "log NOx")
  dev.off()
